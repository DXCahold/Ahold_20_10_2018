			...
			if session["signedin"]:
				...
				if detail["match"]:
					if "not available currently" in request_data["result"]:
						request_data["result"] = request_data["fulfillmentText"].replace(& *offer will be applied on +" is *result!","sorry!").replace("& *offer will be applied on "+& *offer will be applied on +"! would you like to place the order for sure?","")
					else:
